...
export interface OrbSceneApi {
  ...
  resetView(): void;
  dispose(): void;
  setAstaState(s: "idle" | "listening" | "thinking" | "speaking"): void;
}
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  // ——— COLORS ———
  const PALETTES = {
    ULTRON_ORANGE: { C_BRIGHT: 0xffaa30, C_MID: 0xdd7700, C_DIM: 0x884400, C_FAINT: 0x553300, C_HOT: 0xffcc66 },
    ASTA_BLUE: { C_BRIGHT: 0x30aaff, C_MID: 0x0077dd, C_DIM: 0x004488, C_FAINT: 0x002255, C_HOT: 0x66ccff }
  };
  const P = PALETTES.ASTA_BLUE;
  const C_BRIGHT = P.C_BRIGHT;
  const C_MID = P.C_MID;
  const C_DIM = P.C_DIM;
  const C_FAINT = P.C_FAINT;
  const C_HOT = P.C_HOT;
  ...
  function animate() {
    ...
    outerShell.rotation.y += 0.0015 * ASTA_PARAMS.rotSpeed;
    outerShell.rotation.x = Math.sin(t * 0.08) * 0.05 * ASTA_PARAMS.rotSpeed;

    // Panel group follows shell but with slight offset
    panelGroup.rotation.y += 0.0018 * ASTA_PARAMS.rotSpeed;
    panelGroup.rotation.x = Math.sin(t * 0.08 + 0.5) * 0.04 * ASTA_PARAMS.rotSpeed;

    // Secondary shell counter-rotates slowly
    shell2.rotation.y -= 0.001 * ASTA_PARAMS.rotSpeed;
    shell2.rotation.z = Math.sin(t * 0.12) * 0.03 * ASTA_PARAMS.rotSpeed;

    // Inner core — opposite, faster
    innerCore.rotation.y -= 0.005 * ASTA_PARAMS.rotSpeed;
    innerCore.rotation.z += 0.002 * ASTA_PARAMS.rotSpeed;
    innerCore.rotation.x = Math.cos(t * 0.1) * 0.08 * ASTA_PARAMS.rotSpeed;

    // Innermost wireframe
    icoWire.rotation.x += 0.008 * ASTA_PARAMS.rotSpeed;
    icoWire.rotation.y += 0.012 * ASTA_PARAMS.rotSpeed;

    // Core pulse — dramatic surges but mostly transparent
    const wave1 = Math.sin(t * 1.2);
    const wave3 = Math.pow(Math.max(0, Math.sin(t * ASTA_PARAMS.surgeFreq * 0.4)), 5); // rare big surge
    const wave4 = Math.pow(Math.max(0, Math.sin(t * ASTA_PARAMS.surgeFreq * 0.7 + 2)), 8); // mega surge
    const fadeOut = Math.pow(Math.max(0, Math.sin(t * 0.25)), 3); // periodic full transparency
    const surge = (wave3 * 1.5 + wave4 * 2.0) * ASTA_PARAMS.surgeAmp;
    ...
    // Bloom pulse
    bloom.strength = (1.6 + Math.sin(t * 0.8) * 0.3) * ASTA_PARAMS.bloomMult;

    // Update chromatic aberration time
    chromaticPass.uniforms.uTime.value = t;
    chromaticPass.uniforms.uIntensity.value = 0.003 * ASTA_PARAMS.chromaticMult;
