...
class SchedulerService:
    ...
    def __init__(self):
        ...
        Jobs here are bound methods (and, for one-time reminders, callbacks
        holding references back to service instances), which APScheduler's
        MongoDBJobStore cannot pickle. Missed/pending reminders are already
        reconstructed on every restart via `_startup_catch_up` below and the
        Notion reminder reload in main.py, so an in-memory jobstore (jobs
        re-registered fresh on each startup) is sufficient and avoids the
