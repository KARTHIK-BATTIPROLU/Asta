...
## PHASE BOARD
Phase 0: IN-PROGRESS
Phase 1–10: NOT-STARTED

## STATUS MATRIX
| Component | Guide ref | Status | Reason / evidence (file:line) | Disposition |
|---|---|---|---|---|
| settings_routes.py | I.3 | BROKEN | `backend/app/api/settings_routes.py:17` uses deprecated `Depends(get_api_key)` | Fix in Phase 0 |
| metrics_routes.py | I.3 | BROKEN | `backend/app/api/metrics_routes.py:26` uses deprecated `Depends(get_api_key)` | Fix in Phase 0 |
| routine_engine.py | I.3 | BROKEN | `backend/app/workflows/routine_engine.py:118` IndentationError | Fix in Phase 0 |
| ws_transport.py | I.3 | BROKEN | `backend/app/api/ws_transport.py:328` etc has `ctx.ctx.ctx.` chains | Fix in Phase 0 |
| WakeUpActivity.kt | I.3 | PARTIAL | `ASTA MOBILE/app/src/main/java/com/example/asta/alarm/WakeUpActivity.kt:66` lacks view IDs and correct audioStreamer wireup | Fix in Phase 0 |
| memory_saga.py | I.3 | ATTIC | Imports still present in various files; file needs deleting | Fix in Phase 0 |
| .env.template | I.3 | PARTIAL | Missing required config keys | Fix in Phase 0 |
| AstaNetworkClient.kt / ConfigManager.java | I.3 | PARTIAL | Hardcoded ngrok fallbacks exist | Fix in Phase 0 |
...
## NEXT STEP (exact)
- Phase 0 Stabilize & Secure (Part I.3 kill-list): replace `Depends(get_api_key)`, fix `routine_engine.py` indentation, update `ws_transport.py`.
