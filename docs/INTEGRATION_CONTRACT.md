...
> **Crucial Pipecat Direction Semantics Note**: When intercepting user turns (e.g. `TranscriptionFrame`) inside a `FrameProcessor` (like `MemoryContextInjector`), you must check `direction == FrameDirection.DOWNSTREAM`. The frames generated by user audio/text flow *downstream* from the input transport, through STT, to the LLM. Checking for `UPSTREAM` here is a bug and will cause the processor to silently miss all user input in a live pipeline.
